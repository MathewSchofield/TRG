...
papers to discuss:
\begin{itemize}
\item Supervised classifier (classifying variable stars): \citet{debosscher_automated_2007}, \citet{sarro_automated_2009}
\item reverse engineer target selection: \citet{thomas_galactic_2017}
\end{itemize}
...
The feature importances of the x-data labels are given in Figure \ref{fig:feature}. The values show that the label with the highest influence on mode detectability is apparent stellar magnitude \imag. This is not surprising: a higher value of \imag will result in more $\chi^{2}$ 2 Degrees-of-Freedom white noise in the observation. This will make oscillations less likely to be detected above the white noise background.
\begin{figure}
	\centering
