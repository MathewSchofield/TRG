...
\label{sect: prep}
Firstly, the detection probabilities of every mode were taken from Section \ref{sect: det_test}. After removing stars without APOKASC information, this left 811 stars. In Section \ref{sect: dataset}, each star had it's \imag value perturbed 100 times. This left 81,100 stars to perform classification upon. Each calculated detection probability from Section \ref{sect: det_test} was put into a discrete bin (or {\it class}) depending on how likely the mode is to be detected. These discrete classes are given in equation \ref{eq:class}.
\begin{equation}
\label{eq:class}
P_{\rm det} = \left\{ \,
